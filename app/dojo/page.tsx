...
export default function JETTHubPage() {
  ...
  const [animationState, setAnimationState] = useState<"idle" | "panning" | "growing">("idle")
  ...
  // Aspect ratio for AGT divider line correction on mobile only
  ...
  const navigationStates: Record<string, NavigationState> = {
    "jett-hub": {
      ...
      tensor: "hub",
      connections: [
        { id: "gaze-optics", x: 50, y: 20, tensor: "cog", label: "Gaze Optics", icon: Eye },
        { id: "connections", x: 25, y: 75, tensor: "emo", label: "Connections", icon: Users },
        { id: "account", x: 75, y: 75, tensor: "env", label: "Account", icon: Settings },
      ],
    },
    "gaze-optics": {
      id: "gaze-optics",
  ...
  if (!isLoaded) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white">
        <div className="text-xl animate-pulse">Initializing JETT Hub...</div>
      </div>
    )
  }
  ...
  const currentColor = tensorColors[currentState.tensor as keyof typeof tensorColors]

  return (
    <div className="h-screen w-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white overflow-hidden relative">
      <style>{`
        @keyframes electric-flow { 0% { stroke-dashoffset: 0; } 100% { stroke-dashoffset: 20; } }
      ...
      {/* JOE Home button → landing page */}
      <button
        onClick={() => router.push("/")}
        className="absolute top-24 left-8 z-40 bg-transparent border-none cursor-pointer group"
      >
      ...
      <div className="relative h-screen overflow-hidden">
        <div
          ...
          style={{
            transform: `translate(${panOffset.x}vw, ${panOffset.y}vh)`,
        ...
        >
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30">
            <div className="relative">
              <div
                ...
                style={{ backgroundColor: currentColor, opacity: 0.2, transform: "scale(1.5)" }}
              />
              <div
                className="relative w-24 h-24 rounded-full flex items-center justify-center bg-black/80"
                style={{ boxShadow: `0 0 40px ${currentColor}50`, border: `3px solid ${currentColor}` }}
              >
              ...
              <div
                className="absolute -bottom-12 left-1/2 -translate-x-1/2 text-lg font-semibold whitespace-nowrap z-[35]"
          ...
          {(animationState === "idle" || animationState === "growing") &&
            currentState.connections.map((conn) => {
              ...
              const isHovered = hoveredPath === conn.id && animationState === "idle"
              const color = tensorColors[conn.tensor as keyof typeof tensorColors]
              const opacity = animationState === "growing" ? Math.max(0, (growthProgress - 0.8) * 5) : 1
              const scale = animationState === "growing" ? 0.5 + Math.max(0, (growthProgress - 0.8) * 2.5) : 1
              ...
              return (
                <div
                  ...
                  onMouseEnter={() => animationState === "idle" && setHoveredPath(conn.id)}
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{ transform: isHovered ? "scale(1.1)" : "scale(1)", opacity: isHovered ? 1 : 0.6 }}
                    >
                      <div
                      ...
                      >
                        <ConnIcon size={32} style={{ color: isHovered ? color : "#6b7280" }} />
                      </div>
                      <span
                        className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-sm whitespace-nowrap transition-all duration-200"
                        style={{
                          fontWeight: isHovered ? 600 : 400,
                          opacity: isHovered ? 1 : 0.5,
                          color: isHovered ? color : "#d1d5db",
                          textShadow: isHovered ? `0 0 8px ${color}40` : "none",
                        }}
                      >
                        {conn.label}
                      </span>
            ...
            currentState.parentPos && (
              <div
                ...
                style={{
                  ...
                  opacity: animationState === "growing" ? Math.max(0, (growthProgress - 0.8) * 5) : 1,
                  pointerEvents: animationState === "idle" ? "auto" : "none",
                }}
                onMouseEnter={() => animationState === "idle" && setHoveredPath("parent")}
                onMouseLeave={() => setHoveredPath(null)}
              >
                <button
                ...
                >
                  <div
                    className="transition-all duration-300"
                    style={{
                      transform: hoveredPath === "parent" ? "scale(1.1)" : "scale(1)",
                  ...
                  >
                    <div
                      className="w-20 h-20 rounded-full flex items-center justify-center"
                      style={{
                        backgroundColor: hoveredPath === "parent" ? "rgba(0,0,0,0.8)" : "rgba(0,0,0,0.4)",
                        border: `2px solid ${hoveredPath === "parent" ? tensorColors.hub : "rgba(255,255,255,0.1)"}`,
                        boxShadow: hoveredPath === "parent" ? `0 0 20px ${tensorColors.hub}50` : "none",
                      }}
                    >
                      <Eye size={32} style={{ color: hoveredPath === "parent" ? tensorColors.hub : "#6b7280" }} />
                    </div>
                    <span
                      className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-sm whitespace-nowrap transition-all duration-200"
                      style={{
                        ...
                        color: hoveredPath === "parent" ? tensorColors.hub : "#d1d5db",
                      }}
                    >
                      Jett Hub
                    </span>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-40 flex gap-6 items-center">
        {([
          { key: "emo" as const, color: "#ef4444", emoji: "\u2764\uFE0F", label: "EMO" },
          { key: "cog" as const, color: "#facc15", emoji: "\uD83E\uDDE0", label: "COG" },
          { key: "env" as const, color: "#3b82f6", emoji: "\uD83C\uDF0D", label: "ENV" },
        ]).map((t) => (
          <div
            key={t.key}
            ...
            onClick={() => setSelectedTensor(t.key)}
      ...
      {/* Dev mode indicator */}
      {devMode && (
        <div className="absolute bottom-16 left-4 z-50 px-2 py-1 text-xs bg-yellow-500/20 text-yellow-400 rounded font-mono">
          State: {animationState} | View: {currentView}
        </div>
      )}
    </div>
  )
